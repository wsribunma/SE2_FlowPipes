...
class RoverPlanner:
    ...
    def compute_ref_data(self, plot=False):
        ...
        if plot:
            ...
            ax4.set_title('a', fontsize=20)
            ax4.plot(t, f_ref_a(t))
            ax4.grid()
            # ax4.vlines(np.cumsum(self.leg_times), 0.8, 1.2, color='r', alpha=0.5)
            ax4.set_xlabel('t, sec', fontsize=20)
            ax4.set_ylabel('m/s2', fontsize=20)

            ax5.set_title('alpha', fontsize=20)
            ax5.plot(t, np.rad2deg(f_ref_alpha(t)))
            ax5.grid()
            # ax5.vlines(np.cumsum(self.leg_times), -60, 60, color='r', alpha=0.5)
            ax5.set_xlabel('t, sec', fontsize=20)
            ax5.set_ylabel('deg/s2', fontsize=20)
...
#             t_span=[t[0], t[-1]], y0=[X0.x, X0.y, X0.theta], t_eval=t)
    res = scipy.integrate.solve_ivp(
        ...
        y0=[X0.x, X0.y, X0.theta,
            ...
            x0.x, x0.y, x0.theta], args=[ref_data, freq_d, w1, w2, vr, dist, case, use_approx], rtol=1e-9, atol=1e-12)
    return res
...
def plot_rover_sim(res, vr, case, planner, legend=False, save=False, **plt_kwargs):
    ref_data = planner.compute_ref_data()
    t = res['t']
...
#                 r_omega[i,:] = np.pi/4
    ...
    fig1 = plt.figure(1)
    plt.plot(t, (ux), 'c-', label='$u_{\\zeta_x}$ (m/s)' if legend else None, **plt_kwargs)
    plt.plot(t, (uy), 'y-', label='$u_{\\zeta_y}$(m/s)' if legend else None, **plt_kwargs)
    plt.plot(t, (utheta), 'm-', label='$u_{\\zeta_\\theta}$ (rad/s)' if legend else None, **plt_kwargs)
    plt.legend(loc=1)
    plt.grid(True)
...
def plot_sim_corres(res, planner, name=None, legend=False, save=False):
    ...
    fig2 = plt.figure(2)
    title = name + 'Without Exact Log-Linearization'# with Log-Linear Dynamic Inversion Control'#of \n Actual Nonlinear System and Log-Linearized System'
    plt.title(title, fontsize=22)
    plt.grid(True)
